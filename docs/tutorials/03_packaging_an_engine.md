...
*   `You have already developed a custom compression engine (e.g., as a \`.py\` file). For this tutorial, we'll assume you have an engine in \`my_awesome_engine.py\` containing a class \`MyAwesomeEngine\` which inherits from \`CompressionEngine\`.`
*   `Your \`MyAwesomeEngine\` class has a unique string attribute \`id = "awesome_engine"\`.`
## Goal
...
class MyAwesomeEngine(CompressionEngine): # Updated class name
    id = "awesome_engine" # Your unique engine ID
    ...
    def compress(self, text_or_chunks, llm_token_budget, **kwargs):
        ...
        trace = CompressionTrace(
            ...
            engine_params={'custom_param': self.custom_param}, # Updated param
...
```yaml
package_format_version: "1.0"
engine_id: awesome_engine           # Must match the 'id' in your class
...
```
Key changes:
*   `engine_id`: Changed to \`"awesome_engine"\`, matching our class.
...
compact-memory compress --file input.txt --engine awesome_engine --budget 100 --engine-params '{"custom_param": 10}'
```
```python
from CompressionEngine.core import get_compression_engine # Updated import
MyEngineClass = get_compression_engine("awesome_engine") # Updated function and ID
my_engine_instance = MyEngineClass(custom_param=10) # Updated var name
# ... use my_engine_instance.compress(...) ...
```
## Conclusion
