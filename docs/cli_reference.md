# Gist Memory CLI Reference

This document provides a comprehensive reference for the Gist Memory Command Line Interface (CLI).

## Global Options

These options can be used with any command:

*   `--memory-path TEXT`: Path to the Gist Memory agent directory. Overrides environment variables and configuration files. (e.g., `-m ./my_agent_data`)
...
Gist Memory uses a hierarchical configuration system. Settings are resolved in the following order of precedence (highest first):

1.  **Command-Line Arguments:** Options provided directly with a command (e.g., `--memory-path ./my_agent`).
2.  **Environment Variables:**
    *   `GIST_MEMORY_PATH`
    *   `GIST_MEMORY_DEFAULT_MODEL_ID`
    *   `GIST_MEMORY_DEFAULT_STRATEGY_ID`
3.  **Local Project Configuration:** A `.gmconfig.yaml` file in the current working directory.
4.  **User Global Configuration:** A `config.yaml` file located in `~/.config/gist_memory/config.yaml` (path may vary slightly by OS).
5.  **Application Defaults:** Hardcoded default values within the application.

You can manage your global configuration using the `gist-memory config` commands. See the [Configuration Guide](configuration.md) for more details.

## Top-Level Commands

These are the primary commands for interacting with Gist Memory.
...
**Usage:** `gist-memory ingest [OPTIONS] SOURCE`
...
**Usage:** `gist-memory query [OPTIONS] QUERY_TEXT`
...
**Usage:** `gist-memory agent [OPTIONS] COMMAND [ARGS]...`

#### `gist-memory agent init`

Creates and initializes a new agent data store in a specified directory.

**Usage:** `gist-memory agent init [OPTIONS] TARGET_DIRECTORY`
...
Manage Gist Memory application configuration settings.

**Usage:** `gist-memory config [OPTIONS] COMMAND [ARGS]...`

#### `gist-memory config set`

Sets a Gist Memory configuration key to a new value in the user's global config file (`~/.config/gist_memory/config.yaml`).

**Usage:** `gist-memory config set [OPTIONS] KEY VALUE`

**Arguments:**
*   `KEY`: The configuration key to set (e.g., 'gist_memory_path', 'default_model_id'). (Required)
*   `VALUE`: The new value for the configuration key. (Required)

#### `gist-memory config show`

Displays current Gist Memory configuration values, their effective settings, and their sources.

**Usage:** `gist-memory config show [OPTIONS]`
...
**Usage:** `gist-memory dev [OPTIONS] COMMAND [ARGS]...`

#### `gist-memory dev list-metrics`
Lists all available validation metric IDs that can be used in evaluations.
**Usage:** `gist-memory dev list-metrics`
...
**Usage:** `gist-memory dev list-strategies`

#### `gist-memory dev inspect-strategy`
Inspects aspects of a compression strategy, currently focused on 'prototype' strategy's beliefs.
**Usage:** `gist-memory dev inspect-strategy [OPTIONS] STRATEGY_NAME`
**Arguments:**
...
#### `gist-memory dev evaluate-compression`
Evaluates compressed text against original text using a specified metric.
**Usage:** `gist-memory dev evaluate-compression [OPTIONS] ORIGINAL_INPUT COMPRESSED_INPUT`
**Arguments:**
...
#### `gist-memory dev test-llm-prompt`
Tests a Language Model (LLM) prompt with specified context and query.
**Usage:** `gist-memory dev test-llm-prompt [OPTIONS]`
**Options:**
...
#### `gist-memory dev evaluate-llm-response`
Evaluates an LLM's response against a reference answer using a specified metric.
**Usage:** `gist-memory dev evaluate-llm-response [OPTIONS] RESPONSE_INPUT REFERENCE_INPUT`
**Arguments:**
