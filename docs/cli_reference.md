...
*   `--memory-path TEXT`: Path to the Compact Memory data directory (if applicable for the chosen engine). Overrides environment variables and configuration files. (e.g., `-m ./my_memory_data`)
...
*   `--engine-id TEXT`: Default compression engine ID (e.g., `prototype_engine`). Overrides environment variables and configuration files.
...
**Options:**
*   `--engine / -e TEXT`: Compression engine ID to use. Overrides the global default engine. (Required if no global default is set)
...
*   `--alpha FLOAT`: Alpha parameter, controlling the decay rate for memory importance (default: 0.1).
    *   `--chunker TEXT`: Chunking method/algorithm to use for processing text during ingestion (default: "sentence_window").
...
Inspects aspects of a compression engine, currently focused on 'prototype_engine' engine's beliefs.
**Usage:** `compact-memory dev inspect-engine [OPTIONS] ENGINE_NAME`
**Arguments:**
*   `ENGINE_NAME`: The name of the engine to inspect. Currently, only 'prototype_engine' is supported. (Required)
**Options:**
*   `--memory-path TEXT`: Path to the data directory used by the engine (if applicable). Overrides global setting. Required if '--list-prototypes' is used with 'prototype_engine'.
*   `--list-prototypes`: List consolidated prototypes (beliefs) if the engine is 'prototype_engine' and a memory path is provided.
