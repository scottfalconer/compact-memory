...
if __name__ == "__main__":
    ...
    Path("test_params.json").write_text(
        '{"file_key": "file_value", "common_key": "file_common", "file_bool": true, "file_num": 123.45}'
    )
    Path("empty.json").write_text("")
    Path("malformed.json").write_text(
        '{"file_key": "file_value", "common_key": "file_common",}'
    )  # trailing comma

    print(
        f"Test with file only: {parse_complex_params(params_file=Path('test_params.json'))}"
    )
    # Expected: {'file_key': 'file_value', 'common_key': 'file_common', 'file_bool': True, 'file_num': 123.45}

    print(
        f"Test with KV only: {parse_complex_params(params_kv=['kv_key=kv_value', 'common_key=kv_common_override', 'kv_bool=true', 'kv_num=42'])}"
    )
    # Expected: {'kv_key': 'kv_value', 'common_key': 'kv_common_override', 'kv_bool': True, 'kv_num': 42}

    print(
        f"Test with file and KV (KV overrides): {parse_complex_params(params_file=Path('test_params.json'), params_kv=['common_key=kv_override', 'another_kv=123', 'file_bool=false'])}"
    )
    ...
    # Cleanup dummy files
    if Path("test_params.json").exists():
        Path("test_params.json").unlink()
    if Path("empty.json").exists():
        Path("empty.json").unlink()
    if Path("malformed.json").exists():
        Path("malformed.json").unlink()
