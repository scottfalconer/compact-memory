...
if __name__ == "__main__":
        ...
        yaml.dump(
            {
                ...
                "default_strategy_id": "local_strategy",
            },
            f,
        )
    config3 = Config()
    for k, (v, s) in config3.get_all_with_sources().items():
        print(f"{k}: {v} (Source: {s})")
    assert config3.get("default_model_id") == "local/model"
    assert config3.get_with_source("default_model_id")[1] == SOURCE_LOCAL_CONFIG
    assert config3.get("default_strategy_id") == "local_strategy"
    assert config3.get_with_source("default_strategy_id")[1] == SOURCE_LOCAL_CONFIG
    ...
    assert cli_val == "cli/model"
    assert cli_src == "command-line argument"
    # Test CLI override for a boolean
    config_cli.update_from_cli(
        "some_bool_not_in_defaults_yet", "true"
    )  # Assuming it might be added later
    # This will be added as a new key with "command-line argument" source

    print("\n--- Test 6: Set and Persist (User Global) ---")
    if USER_CONFIG_PATH.exists():
        os.remove(USER_CONFIG_PATH)  # Clean user config
    if LOCAL_CONFIG_PATH.exists():
        os.remove(LOCAL_CONFIG_PATH)  # Clean local
    if ENV_VAR_PREFIX + "DEFAULT_MODEL_ID" in os.environ:
        del os.environ[ENV_VAR_PREFIX + "DEFAULT_MODEL_ID"]
    ...
    assert val_set == "set/model"
    assert src_set == SOURCE_OVERRIDE
    ...
    print(
        f"Reloaded after set: default_model_id = {val_reloaded} (Source: {src_reloaded})"
    )
    assert val_reloaded == "set/model"
    assert src_reloaded == SOURCE_USER_CONFIG
    ...
    print(
        f"After invalid set: default_model_id = {val_invalid} (Source: {src_invalid})"
    )
