...
def test_pkvp_type_conversion():
    ...
    assert parse_key_value_pairs(
        ["int=123", "float=45.6", "bool_true=true", "bool_false=False", "null_val=null"]
    ) == {
        "int": 123,
        "float": 45.6,
        "bool_true": True,
...
def test_pkvp_json_values():
    ...
    }  # This is treated as a plain string
...
def test_pkvp_invalid_format():
    with pytest.raises(
        typer.BadParameter,
        match="Invalid parameter format 'key_no_value'. Expected 'key=value'.",
    ):
        parse_key_value_pairs(["key_no_value"])
    with pytest.raises(
        typer.BadParameter, match="Invalid parameter format ''. Expected 'key=value'."
    ):
        parse_key_value_pairs([""])  # Empty string in list
    with pytest.raises(
        typer.BadParameter,
        match="Invalid parameter format ' =value_no_key'. Expected 'key=value'.",
    ):
        parse_key_value_pairs([" =value_no_key"])
    with pytest.raises(
        typer.BadParameter, match="Failed to decode JSON value for key 'bad_json'"
    ):
...
def test_pcp_file_and_kv_no_override(temp_json_file: Path):
    ...
    assert (
        parse_complex_params(params_file=temp_json_file, params_kv=kv_data) == expected
    )
...
def test_pcp_file_and_kv_with_override(temp_json_file: Path):
    ...
    kv_data = [
        'common_key="from_kv"',
        "kv_only_key=kv_val",
    ]  # Ensure "from_kv" is treated as a string
    expected = {
        "common_key": "from_kv",
        "file_only_key": "file_val",
        "kv_only_key": "kv_val",
    }
    assert (
        parse_complex_params(params_file=temp_json_file, params_kv=kv_data) == expected
...
def test_pcp_file_not_found():
    ...
    with pytest.raises(
        typer.BadParameter, match=f"Parameters file not found: {non_existent_file}"
    ):
...
def test_pcp_malformed_json_file(temp_json_file: Path):
    ...
    with pytest.raises(
        typer.BadParameter, match=f"Error decoding JSON from '{temp_json_file}'"
    ):
...
def test_pcp_json_file_not_dict(temp_json_file: Path):
    ...
    with pytest.raises(
        typer.BadParameter,
        match=f"Parameters file '{temp_json_file}' must contain a JSON object \\(dictionary\\).",
    ):
        parse_complex_params(params_file=temp_json_file, params_kv=None)
