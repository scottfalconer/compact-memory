...
from CompressionEngine.contrib.active_memory_engine import ( # Updated path
    ActiveMemoryEngine, # Updated class name
    ActiveMemoryManager, # Assuming ActiveMemoryManager remains in this module or its path is updated accordingly
)
from CompressionEngine.core.trace import CompressionTrace # Updated path
...
def test_instantiation_default_params() -> None:
    engine = ActiveMemoryEngine() # Updated variable and class name
    assert isinstance(engine.manager, ActiveMemoryManager)
    assert engine.manager.config_max_history_buffer_turns == 100
    assert engine.id == "active_memory_engine" # Updated id


def test_instantiation_custom_params() -> None:
    engine = ActiveMemoryEngine( # Updated class name
        config_max_history_buffer_turns=5,
        config_activation_decay_rate=0.5,
    )
    assert engine.manager.config_max_history_buffer_turns == 5
    assert engine.manager.config_activation_decay_rate == 0.5
...
def test_add_single_turn() -> None:
    ...
    engine.add_turn(text, turn_embedding=embedding) # Updated variable name

    assert len(engine.manager.history) == 1
    added = engine.manager.history[0]
    assert added.text == text
    assert added.turn_embedding == embedding
    assert added.current_activation_level == engine.manager.config_initial_activation


def test_compress_empty_history(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "CompressionEngine.contrib.active_memory_engine.embed_text", # Updated path
        mock_embed_texts_func,
    )
    engine = ActiveMemoryEngine() # Updated variable and class name
    query = "Test query"
    compressed, trace = engine.compress(query, 50, tokenizer=mock_tokenizer_func) # Updated variable name
...
def test_compress_simple_history_fits_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    engine = ActiveMemoryEngine(config_prompt_num_forced_recent_turns=2) # Updated variable and class name
    info1 = "Old important info"
    info2 = "Recent relevant info"
    engine.add_turn(info1, turn_embedding=get_mock_embedding(info1)) # Updated variable name
    engine.add_turn(info2, turn_embedding=get_mock_embedding(info2)) # Updated variable name

    compressed, _ = engine.compress("Query text", 20, tokenizer=mock_tokenizer_func) # Updated variable name
...
def test_compress_respects_token_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    engine = ActiveMemoryEngine(config_prompt_num_forced_recent_turns=0) # Updated variable and class name
    engine.add_turn( # Updated variable name
    ...
    engine.add_turn("Short three.", turn_embedding=get_mock_embedding("Short three.")) # Updated variable name

    compressed, _ = engine.compress("Query", 5, tokenizer=mock_tokenizer_func) # Updated variable name
...
def test_compress_pruning_max_history(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    engine.add_turn(t1, trace_strength=0.1, turn_embedding=get_mock_embedding(t1)) # Updated variable name
    engine.add_turn(t2, trace_strength=1.0, turn_embedding=get_mock_embedding(t2)) # Updated variable name
    engine.add_turn(t3, trace_strength=1.0, turn_embedding=get_mock_embedding(t3)) # Updated variable name

    assert len(engine.manager.history) == 2
    texts = [t.text for t in engine.manager.history]
    assert t1 not in texts
    assert t2 in texts
    assert t3 in texts

    compressed, _ = engine.compress("Query", 50, tokenizer=mock_tokenizer_func) # Updated variable name
