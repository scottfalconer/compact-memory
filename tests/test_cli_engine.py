...
def test_engine_list(tmp_path: Path, patch_embedding_model):
    ...
    for engine_id in registered_ids:
        if engine_id not in ["dummy_trunc"]: # Exclude test-specific, dynamically registered ones if not always present
...
def test_engine_info(tmp_path: Path, patch_embedding_model):
    ...
    assert "not found" in result_non_existent.stderr.lower() # Check stderr for error message
...
def test_engine_init_success(tmp_path: Path, patch_embedding_model):
    ...
    tau_value = 0.78 # Unique value for this test
...
def test_engine_clear(tmp_path: Path, patch_embedding_model):
    ...
    clear_result = runner.invoke(app, ["engine", "clear", "--memory-path", str(store_path), "--force"], env=_env(tmp_path))
