...
def test_semantic_comprehension_us1(neocortex_engine: NeocortexTransfer):
    ...
    assert "related_prior_knowledge" not in comprehended_info # "The" is not in prior_knowledge
    ...
    assert comprehended_info2["related_prior_knowledge"] == "A small carnivorous mammal."
...
def test_short_term_retention_us2(neocortex_engine: NeocortexTransfer):
    ...
    comprehended_info = {"main_idea": "A key concept", "tokens": ["A", "key", "concept"]}
...
def test_encoding_to_ltm_us3(neocortex_engine: NeocortexTransfer):
    ...
    assert ltm_trace["encoding_context"]["original_text"] == "This is an important fact to remember."
...
def test_consolidation_us4(neocortex_engine: NeocortexTransfer):
    ...
    retained1 = {"chunk_content": "fox information", "stm_strength": 1.0, "source_comprehension": {"original_text": "fox details"}}
    trace1 = neocortex_engine._encode_to_long_term_memory(retained1)
    trace1["salience"] = 0.8 # Make it more salient

    retained2 = {"chunk_content": "moon facts", "stm_strength": 1.0, "source_comprehension": {"original_text": "moon details"}}
    neocortex_engine._encode_to_long_term_memory(retained2) # trace_id 1

    retained3 = {"chunk_content": "another fox note", "stm_strength": 1.0, "source_comprehension": {"original_text": "more fox details"}}
    neocortex_engine._encode_to_long_term_memory(retained3) # trace_id 2


    neocortex_engine.trigger_consolidation_phase(cycles=5) # Multiple cycles to ensure consolidation
    ...
    assert consolidated_trace1["consolidation_level"] > 0.5 # Should have increased significantly
    ...
    if consolidated_trace1["consolidation_level"] >= 0.8:
        ...
        assert 0 not in consolidated_trace2.get("linked_traces", []) # trace1 and trace2 no common words
...
def test_retrieval_us5(neocortex_engine: NeocortexTransfer):
    ...
    assert "Content: 'The quick brown fox'" in retrieved_fox
    assert "ID: 0" in retrieved_fox
    assert "Confidence:" in retrieved_fox
    # Check working memory update
    assert "The quick brown fox" in engine.working_memory_context

    retrieved_moon = engine.decompress("moon mission")
    assert "Content: 'A mission to the'" in retrieved_moon # "the" is from main_idea (5 words)
...
def test_full_flow_compress_consolidate_decompress(neocortex_engine: NeocortexTransfer):
    ...
    assert compression_result["content"] == "The diligent student studies cognitive" # First 5 words
