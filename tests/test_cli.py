...
def test_cli_agent_stats(tmp_path):
    ...
    result = runner.invoke(app, ["--memory-path", str(tmp_path), "agent", "stats", "--json"])
    assert result.exit_code == 0
...
def test_cli_agent_validate_and_clear(tmp_path):
    ...
    result = runner.invoke(app, ["--memory-path", str(tmp_path), "agent", "clear", "--force"]) # --yes to --force
    assert result.exit_code == 0
    assert not tmp_path.exists() # This check might be too strong if clear only empties dir
...
def test_cli_query(tmp_path, monkeypatch): # Renamed from test_cli_talk
    ...
    assert init_result.exit_code == 0, f"Agent init failed: {init_result.stdout + init_result.stderr}"
    ...
    class DummyChatModel: # Renamed class for clarity
        ...
        model = type("M", (), {"config": type("C", (), {"n_positions": 50})()})() # Simplified mock
        max_new_tokens = 10

        def load_model(self):
            pass

        def prepare_prompt(self, agent_instance, prompt_text, **kw): # Matched expected signature if any
            return prompt_text
    ...
    result = runner.invoke(
        app,
        [
            "--memory-path", str(tmp_path), # Global option
            "query",
            "hi?" # query_text argument
        ]
    )
    assert result.exit_code == 0, f"Query command failed: {result.stdout + result.stderr}"
    assert "mocked_response" in result.stdout
    assert "hello world" in prompts["text"], "Prompt given to LLM did not contain expected memory content"
    assert "User asked: hi?" in prompts["text"], "Prompt given to LLM did not contain the user's query correctly formatted"
...
def test_query_command_calls_receive_channel_message(tmp_path, monkeypatch): # Renamed
    ...
    class DummyAgent:
        store = type("S", (), {"meta": {"default_model_id": "mock_model"}})() # Added default_model_id to meta
        ...
        def receive_channel_message(self, channel_id: str, text: str, mgr: Any, compression: Any = None):
...
def test_cli_download_chat_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "transformers.AutoTokenizer.from_pretrained", DummyModelAutoClasses.from_pretrained
    )
    monkeypatch.setattr(
        "transformers.AutoModelForCausalLM.from_pretrained", DummyModelAutoClasses.from_pretrained
    )

    result = runner.invoke(app, ["dev", "download-chat-model", "--model-name", "foo_model_test"]) # Changed command path & model name
    assert result.exit_code == 0, f"dev download-chat-model failed: {result.stdout + result.stderr}"
...
def test_cli_logging(tmp_path):
    ...
    result = runner.invoke(
        ...
        [
            ...
            "--verbose",
            "--memory-path", str(tmp_path), # Global memory path
            "agent", "stats", # Command
            # No need for command-specific --memory-path-arg here as global is set
        ],
    )
    assert result.exit_code == 0, f"agent stats with logging failed: {result.stdout + result.stderr}"
    ...
    assert "alpha_logging_test" in log_content or "prototypes" in log_content or "DEBUG" in log_content
...
def test_cli_dev_inspect_trace(tmp_path): # Renamed
    ...
    assert result.exit_code == 0, f"dev inspect-trace failed: {result.stdout + result.stderr}"
    ...
    assert "test" in result.stdout # Check if the reason "test" from details is in the output


def test_cli_summarize_string_stdout(): # Renamed
    runner = CliRunner()
    result = runner.invoke(
        app,
        ["summarize", "alpha bravo charlie delta", "--strategy", "none", "--budget", "3"], # compress -> summarize
    )
    assert result.exit_code == 0, f"summarize string stdout failed: {result.stdout + result.stderr}"
    assert "alpha bravo charlie" in result.stdout


def test_cli_summarize_string_to_file(tmp_path): # Renamed
    runner = CliRunner()
    ...
    result = runner.invoke(
        ...
        [
            ...
            "--strategy", "none",
            "--budget", "3",
            "-o", str(out_file), # --output / -o option
        ],
    )
    assert result.exit_code == 0, f"summarize string to file failed: {result.stdout + result.stderr}"
    assert out_file.read_text() == "alpha bravo charlie"


def test_cli_summarize_file(tmp_path): # Renamed
    runner = CliRunner()
    ...
    assert result.exit_code == 0, f"summarize file failed: {result.stdout + result.stderr}"
    assert out_file.read_text() == "one two"


def test_cli_summarize_directory(tmp_path): # Renamed
    runner = CliRunner()
    dir_in = tmp_path / "inputs"
    dir_in.mkdir()
    (dir_in / "a.txt").write_text("alpha bravo charlie")
    (dir_in / "b.txt").write_text("delta echo foxtrot")
    # For directory processing, summarize command saves output files in the same directory
    # with _compressed suffix if -o is not a directory.
    result = runner.invoke(
        app,
        ["summarize", str(dir_in), "--strategy", "none", "--budget", "2"], # compress -> summarize
    )
    assert result.exit_code == 0, f"summarize directory failed: {result.stdout + result.stderr}"
    assert (dir_in / "a_compressed.txt").read_text() == "alpha bravo"
    assert (dir_in / "b_compressed.txt").read_text() == "delta echo"


def test_cli_summarize_invalid_strategy(): # Renamed
    runner = CliRunner()
    result = runner.invoke(
        app,
        ["summarize", "text", "--strategy", "bogus_strategy_name", "--budget", "3"], # compress -> summarize
    )
    assert result.exit_code != 0, f"summarize with invalid strategy did not fail as expected: {result.stdout + result.stderr}"
    # Error message might have changed slightly, ensure it indicates strategy issue
    assert "Unknown compression strategy" in result.stderr or "Invalid value for '--strategy'" in result.stderr


def test_cli_summarize_output_trace(tmp_path): # Renamed
    runner = CliRunner()
    trace_file = tmp_path / "trace.json"
    result = runner.invoke(
        app,
    ...
    assert result.exit_code == 0, f"summarize with --output-trace failed: {result.stdout + result.stderr}"
...
def test_cli_dev_list_strategy_and_metric(): # Renamed
    runner = CliRunner()
    res = runner.invoke(app, ["dev", "list-strategies"]) # strategy list -> dev list-strategies
...
def test_cli_dev_evaluate_compression(): # Renamed
    ...
    res = runner.invoke(
        app,
        [
            "dev", "evaluate-compression", # Added dev
            "abcde",
            "abcd",
    ...
    assert res.exit_code == 0, f"dev evaluate-compression failed: {res.stdout + res.stderr}"
...
def test_cli_dev_test_llm_prompt(monkeypatch, tmp_path): # Renamed
    ...
    class DummyProvider:
        def generate_response(self, prompt, model_name, max_new_tokens, api_key=None, **kw): # Added api_key
    ...
    cfg_file.write_text("modelx:\n  provider: openai\n  model_name: modelx_openai_name") # Made model name more specific
    res = runner.invoke(
        app,
        [
            "dev", "test-llm-prompt", # Added dev
            "--context", "foo_context",
            "--query", "bar_query",
            "--model", "modelx", # This is the key in llm_models_config.yaml
            "--llm-config", str(cfg_file),
        ],
    )
...
def test_cli_dev_evaluate_llm_response(): # Renamed
    ...
    res = runner.invoke(
        app,
        [
            "dev", "evaluate-llm-response", # Added dev
    ...
    assert res.exit_code == 0, f"dev evaluate-llm-response failed: {res.stdout + res.stderr}"
    data = json.loads(res.stdout.strip())
...
def test_cli_config_show_defaults(tmp_path, monkeypatch):
    ...
    effective_user_config_file = Path(str(SUT_USER_CONFIG_PATH).replace("~", str(tmp_path)))
    ...
    for key, default_value in DEFAULT_CONFIG.items():
        ...
        for line in result.stdout.splitlines():
            if key in line:
                key_line = line
                break
        assert "application default" in key_line, f"Source for {key} not 'application default' or key not found in output line."
...
def test_cli_config_set_and_show_specific_key(tmp_path, monkeypatch):
    ...
    assert result_set.exit_code == 0, f"config set failed for {key_to_set}: {result_set.stdout}{result_set.stderr}"
    ...
    result_set_verbose = runner.invoke(app, ["config", "set", key_verbose, value_verbose_str])
    assert result_set_verbose.exit_code == 0, f"config set failed for {key_verbose}: {result_set_verbose.stdout}{result_set_verbose.stderr}"

    with open(effective_user_config_file, "r") as f:
        content_verbose = yaml.safe_load(f)
    assert content_verbose[key_verbose] is expected_stored_bool # Should be stored as boolean by config.set
    ...
    assert str(expected_stored_bool) in result_show_verbose.stdout # Output should reflect boolean
...
def test_cli_config_show_with_env_override(tmp_path, monkeypatch):
    ...
    from gist_memory.config import USER_CONFIG_PATH as SUT_USER_CONFIG_PATH, ENV_VAR_PREFIX
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    effective_user_config_file = Path(str(SUT_USER_CONFIG_PATH).replace("~", str(tmp_path)))
    effective_user_config_file.parent.mkdir(parents=True, exist_ok=True)
    ...
    assert result.exit_code == 0, f"config show with env override failed: {result.stdout}{result.stderr}"

    assert user_setting_key in result.stdout
    assert env_value in result.stdout # Env value should be shown
    assert f"environment variable ({env_key_name})" in result.stdout # Source should be env var
...
def test_cli_config_set_invalid_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    effective_user_config_file = Path(str(SUT_USER_CONFIG_PATH).replace("~", str(tmp_path)))
    effective_user_config_file.parent.mkdir(parents=True, exist_ok=True) # Ensure dir exists

    result = runner.invoke(app, ["config", "set", "this_is_an_invalid_key", "some_value"])
    assert result.exit_code != 0 # Should fail
    # Check stderr for the error message (cli.py prints error from config.set to typer.secho with err=True)
    assert "Error: Configuration key 'this_is_an_invalid_key' is not a recognized setting." in result.stderr
...
def test_cli_interactive_prompt_for_memory_path_if_tty_and_missing(tmp_path, monkeypatch):
    runner = CliRunner()
    ...
    assert result.exit_code != 0 # agent stats should fail on an empty/non-existent path
    # The error message from _load_agent in cli.py when path doesn't exist
    assert f"Error: Memory path '{prompted_path}' not found or is invalid." in result.stderr
...
def test_cli_no_interactive_prompt_if_not_tty_and_missing_path(tmp_path, monkeypatch):
    ...
    result = runner.invoke(app, ["agent", "stats"]) # Use a command that requires memory_path

    assert result.exit_code != 0 # Should fail due to missing path
    assert not prompt_called # Verify typer.prompt was NOT called
    assert "Error: Gist Memory path is not set." in result.stderr # Specific non-interactive error
    assert "Please set it using the --memory-path option" in result.stderr

    monkeypatch.setattr(typer, "prompt", original_typer_prompt) # Restore original prompt
