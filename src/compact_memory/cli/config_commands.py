...
def set_config_command(  # Renamed from config_set_command
    ...
    try:
        ...
        success = config.set(
            key, value
        )  # This now directly calls the Config instance method
        if success:
            typer.secho(
            ...
            raise typer.Exit(
                code=1
            )  # Exit if not successful and no specific error was printed by config.set
    except (
        ValueError
    ) as e:  # Catch validation errors from Pydantic models within Config.set
...
def show_config_command(  # Renamed from config_show_command
    ...
    table.add_column(
        "Effective Value", style="magenta", overflow="fold"
    )  # Allow folding for long values
    table.add_column(
        "Source", style="green", no_wrap=True, overflow="fold"
    )  # Allow folding for long source paths

    if key:
        if key not in config.get_all_keys():  # Check if the key is known at all
            typer.secho(
                f"Error: Configuration key '{key}' is not a recognized key.",
                fg=typer.colors.RED,
                err=True,
            )
        ...
        if (
            value is not None
        ):  # Or always show, even if value is None but source indicates "default"
            table.add_row(
                key,
                str(value) if value is not None else "Not Set (uses default)",
                source_info,
            )
    ...
    if table.row_count > 0:
        ...
        typer.echo(
            "No configuration settings found to display."
        )  # Message if table is empty and no specific key was searched
