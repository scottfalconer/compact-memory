...
from .compress_commands import compress_command # Assuming it's named compress_command now
from .ingest_commands import ingest_command # Assuming it's named ingest_command now
...
app.command("ingest")(ingest_command) # This will show the deprecation message
...
@app.callback(invoke_without_command=True) # invoke_without_command=True allows global options even if no subcommand is given
def main_callback( # Renamed from main to main_callback for clarity
    ...
    memory_path: Optional[str] = typer.Option( # Keep as str, resolve after Config loads
        None,
        "--memory-path",
        "-m",
    ...
    engine_id: Optional[str] = typer.Option( # This is for default history compression / one-shot compression
        None,
        "--engine",
    ...
    resolved_log_file = log_file if log_file else Path(config.get("log_file")) if config.get("log_file") else None
    resolved_verbose = verbose if verbose is not None else config.get("verbose", False) # Default to False if not in config
    ...
    if resolved_log_file:
        ...
        configure_logging(str(resolved_log_file), level) # configure_logging expects string path
    ...
    if memory_path: # CLI option for memory_path is directly provided
        ...
        if resolved_memory_path_str: # Ensure it's absolute if loaded from config/env
             resolved_memory_path_str = str(Path(resolved_memory_path_str).expanduser().resolve())
    ...
    resolved_engine_id = engine_id if engine_id is not None else config.get("default_engine_id")
    ...
    try:
        ...
        logging.error(f"Error during plugin loading or enabling experimental engines: {e}")
    ...
    if current_command_name:
        ...
        commands_not_needing_memory_path_strict = ["config", "dev", "ingest", "version"] # "version" is handled by callback

        # Check if the current command or its parent group is in the exclusion list
        requires_memory_path_check = True
        if any(current_command_name.startswith(cmd_prefix) for cmd_prefix in commands_not_needing_memory_path_strict):
            requires_memory_path_check = False

        # Specific subcommands of 'dev' or 'engine' that don't need a path
        if current_command_name in [
            "dev.list-metrics", "dev.list-engines", "dev.list-strategies",
            "dev.create-engine-package", "dev.validate-engine-package",
            "dev.download-embedding-model", "dev.download-chat-model",
            "engine.list", "engine.info" # These list available, don't operate on a store
        ]:
            requires_memory_path_check = False

        # Compress to stdout/file doesn't need a memory_path
        if current_command_name == "compress" and not memory_path: # if compress is called without --memory-path global opt
            requires_memory_path_check = False


        if requires_memory_path_check and not resolved_memory_path_str:
            is_interactive = sys.stdin.isatty() and sys.stdout.isatty()
            prompt_default_path_str = config.get_default("compact_memory_path") # Get schema default

            if is_interactive:
                typer.secho("The Compact Memory path (engine store location) is not set.", fg=typer.colors.YELLOW)
                new_path_input = typer.prompt(
                    "Please enter the path for Compact Memory storage",
                    default=(str(Path(prompt_default_path_str).expanduser()) if prompt_default_path_str else None),
                )
                if new_path_input:
                    resolved_memory_path_str = str(Path(new_path_input).expanduser().resolve())
                    ctx.obj["compact_memory_path"] = resolved_memory_path_str # Update context
                    typer.secho(f"Using memory path: {resolved_memory_path_str}", fg=typer.colors.GREEN)
                    typer.echo(f'To set this path permanently, run: compact-memory config set compact_memory_path "{resolved_memory_path_str}"')
                else:
                    typer.secho("Memory path is required to proceed with this command.", fg=typer.colors.RED, err=True)
                    raise typer.Exit(code=1)
            else: # Non-interactive, path required but not set
                typer.secho("Error: Compact Memory path is not set and required for this command.", fg=typer.colors.RED, err=True)
                typer.secho("Set via --memory-path, COMPACT_MEMORY_PATH env var, or 'compact-memory config set compact_memory_path ...'.", err=True)
                raise typer.Exit(code=1)
