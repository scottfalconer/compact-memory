...
from compact_memory.engines import load_engine, get_compression_engine # Added get_compression_engine
from compact_memory import local_llm # Assuming this is the correct module for LocalChatModel
from compact_memory.contrib import ActiveMemoryManager # Assuming this is the correct module
...
from compact_memory.engines.registry import get_engine_metadata, available_engines # For history compression engine


query_app = typer.Typer(help="Query the Compact Memory engine store.") # Can be a Typer app or just hold commands
...
def query_command( # Renamed from query
    ...
    try:
        ...
        logging.exception(f"Failed to load engine from {resolved_memory_path}") # Keep detailed log
        raise typer.Exit(code=1)
    ...
    if hasattr(engine_instance, "_chat_model"): # Check if the attribute exists
        try:
            ...
            engine_instance._chat_model = local_llm.LocalChatModel(model_name=final_model_id)
    ...
    if final_history_compression_engine_id and final_history_compression_engine_id.lower() != "none":
        try:
            CompressionEngineCls = get_compression_engine(final_history_compression_engine_id) # Renamed variable
            engine_meta_info = get_engine_metadata(final_history_compression_engine_id) # Renamed variable
            if engine_meta_info and engine_meta_info.get("source") == "contrib": # Check source if available
                typer.secho(
                    "\u26a0\ufe0f Using experimental engine for history compression from contrib: not officially supported.",
                    fg=typer.colors.YELLOW,
                )
            history_comp_engine_instance = CompressionEngineCls() # Instantiate the class
    ...
    try:
        if not hasattr(engine_instance, "receive_channel_message"):
            typer.secho(f"Error: The loaded engine type '{type(engine_instance).__name__}' does not support the 'receive_channel_message' method required for querying.", fg=typer.colors.RED, err=True)
            raise typer.Exit(code=1)
        ...
        try:
            query_result = engine_instance.receive_channel_message( # Renamed variable
                "cli", query_text, active_memory_mgr, compression=history_comp_engine_instance
            )
        except TypeError as e:
            if "got an unexpected keyword argument 'compression'" in str(e) or \
               "takes at most 3 positional arguments" in str(e) or \
               "takes from 3 to 4 positional arguments but 5 were given" in str(e): # Adjusted error check
                 typer.secho(
                 ...
                 query_result = engine_instance.receive_channel_message("cli", query_text, active_memory_mgr)
            else:
